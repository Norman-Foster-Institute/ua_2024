...
As above, variables can store different kinds of data, including `str`, `int`, `float` (decimal), and other types such as "booleans" (`True` and `False`) and `None` types. If you want to find out what `type` a variable is, you can use the built-in `type()` method, which will tell you what `type` a given variable is.
...
`shapely` let's us do some neat things with spatial information. Let's define a `Point` type using the `geometry` module. The `geometry` module has some already defined methods which we can call. Let's use the `Point` method to create a new point. Note the use of dot-notation.
...
For example, a `shapely` `Point` type has built-in methods which allow you to easily produce certain forms of behaviour. You can access these using dot-notation. Let's buffer the point object using the pre-defined `buffer()` method.
...
Notice that we now have a `Polygon` type. Types (or a Python `class`) can have properties which are accessed using dot-notation, but unlike methods, properties do not require parenthesis because they do not accept arguments.
...
Let's create a line. The line constructor requires a list of points.
