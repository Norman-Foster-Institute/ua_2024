...
- Click on the first entry (where all data has been aggregated as `TODOS`, in several fields) then hold down shift and click on the last entry with a `fecha` value of `1 de enero de 2023`.
- Click the `Invert selection` button.
- Toggle editing.
- Click the `Delete selected features` button.
- As before, create a new field using the Field Calculator: `num_pple` (as `Integer` type).
- Use the following expression to populate the field : `to_real("num_personas")`
